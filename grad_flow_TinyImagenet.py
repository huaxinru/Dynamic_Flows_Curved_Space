...
data_path = "./data/TinyImagenet_SVHN.tar"
device = 'cpu'
data = torch.load(data_path,map_location=torch.device(device))
source_data = torch.from_numpy(data["source_data"]).to(device).float()
source_mean = torch.from_numpy(data["source_mean"]).to(device).float()
source_cov = torch.from_numpy(data["source_cov"]).to(device).float()


target_data = torch.from_numpy(data["target_data"]).to(device).float()
target_data_cpu = torch.from_numpy(data["target_data"]).float().cpu().numpy()
target_mean = torch.from_numpy(data["target_mean"]).to(device).float()
target_cov = torch.from_numpy(data["target_cov"]).to(device).float()
...
K_cov = target_cov[start_class:end_class]
...
path = "results_I_C/1_shot/"+dt_string+"/"
...
with open(path+"setup.txt", "w") as f:
    ...
    f.write(f"start class: {start_class}, end class: {end_class}\n")
    f.write(f"alpha: {alpha}, beta: {beta}, gamma: {gamma}, step_size: {step_size}\n")
    f.write("reduce stepsize to 1/(1+k/100) every 40 step")
    if momentum_flag:
...
for c in range(num_classes):
    for i in range(one_class):
        index = (c+start_class)*data_one_class+i
...
save_tensor_images_cifar(torch.reshape(x_tau,(num_images,image_shape[0],image_shape[1],image_shape[2])),path, "init_source",num_images)
save_tensor_images_cifar(torch.reshape(K_x_tau,(num_target_images,image_shape[0],image_shape[1],image_shape[2])),path, "init_target", num_images)


start_time = time.time()
one_batch_num = int(num_images/4)
with torch.no_grad():
    for k in range(num_steps):
        if k%4==0:
            batches = indices[:one_batch_num]
        elif k%4==1:
            batches = indices[one_batch_num:2*one_batch_num]
        elif k%4==2:
            batches = indices[2*one_batch_num:3*one_batch_num]
        else:
            batches = indices[3*one_batch_num:].copy()
            np.random.shuffle(indices)
        #select the minibatch
        if k%100==0 and k!=0:
            step_size = step_size_0/(1+k/100)
        ...
        for i in batches:
            ...
            noise = torch.randn(image_dim, device = device)*noise_beta
        ...
        # save flow results every 40 steps
        if k %40==0:
            ...
            source_mean_plt = []
            save_tensor_images_cifar(x_tau.reshape(num_images,image_shape[0],image_shape[1],image_shape[2]), path, k, num_images, image_shape)

# save plots and results in .tar
            results_tau = {}
            ...
            plt.clf()
            plt.plot(
                np.arange(1, int(len(objectives))*20+1, 20),
            ...
            with open(path+"setup.txt", "a") as f:
                f.write("--- %s steps,  %s seconds ---" % (k, time.time() - start_time))
